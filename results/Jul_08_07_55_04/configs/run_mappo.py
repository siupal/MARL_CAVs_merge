...
# 禁用Gym的环境检查器，解决兼容性问题
import gym
gym.logger.set_level(40)  # 只显示错误日志
os.environ['PYTHONWARNINGS'] = 'ignore:Trying to detect:UserWarning:gym'
# 禁用环境检查器
...
import os
from gym_compatibility import GymCompatibilityWrapper
...
def train(args):                                                                                    # 训练
    ...
    # 在开始交互前先显式调用一次reset方法初始化环境
    reset_result = mappo.env.unwrapped.reset() if hasattr(mappo.env, 'unwrapped') else mappo.env.reset()
    ...
    if isinstance(reset_result, tuple):
        if len(reset_result) == 2:
            if isinstance(reset_result[1], dict):
                ...
                mappo.env_state = reset_result[0]
                mappo.action_mask = reset_result[1].get("action_mask", None)
            else:
                # 旧API格式: (observation, action_mask)
                mappo.env_state = reset_result[0]
                mappo.action_mask = reset_result[1]
        else:
            # 其他格式的tuple
            mappo.env_state = reset_result[0]
            mappo.action_mask = None
    else:
        # 单一返回值
        mappo.env_state = reset_result
        mappo.action_mask = None
