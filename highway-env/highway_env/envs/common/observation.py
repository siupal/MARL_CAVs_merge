...
class ObservationType(object):                                                          #观察类型
    def __init__(self, env: 'AbstractEnv', **kwargs) -> None:                           #初始化    #AbstractEnv是一个抽象类  #**kwargs是一个字典  #-> None表示返回值为空
        self.env = env
        self.__observer_vehicle = None                                                  #观察车辆

    def space(self) -> spaces.Space:                                                    #空间     #spaces.Space是一个类   #-> spaces.Space表示返回值是一个spaces.Space类型
...
class KinematicObservation(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        if df.shape[0] < self.vehicles_count:
            ...
            new_rows_df = pd.DataFrame(data=rows, columns=self.features)
